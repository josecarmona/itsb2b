...
class modificacion_name_get(models.Model):
    ...
    def name_get(self):
        result = []
        for res in self:
            rate = '{0:,.2f}'.format(res.rate)
            rate = (str(rate)).replace('.', ',')
            name = str(rate) + ' / ' + str(res.name) + ' / ' + str(res.currency_id.display_name)
...
class RetentionISLR(models.Model):
    ...
    def amount_initial1(self, account_account, show_accounts, balance, start_date, end_date, company, state, currency, tasa):
        ...
        for row in cr.dictfetchall():
            ...
            for line in move_lines.get(row['account_id']):
                if currency.id == 2 or currency.id == 1:
                    if currency_line.rate > 0:
                        balance += (line['debit'] - line['credit']) / currency_line.rate
                else:
        ...
        for account in account_account:
            ...
            for line in res.get('move_lines'):
                if currency.id == 2 or currency.id == 1:
                    res['debit'] += line['debit'] / currency_line.rate if line['lname'] != 'Initial Balance' else False
                    res['credit'] += line['credit'] / currency_line.rate if line['lname'] != 'Initial Balance' else False
                    res['balance'] += ((line['debit'] - line['credit'])/currency_line.rate)
              #      res.update({'balance': (line['balance']/currency_line.rate)})


                else:
                    res['debit'] += line['debit'] if line['lname'] != 'Initial Balance' else False
                    res['credit'] += line['credit'] if line['lname'] != 'Initial Balance' else False
    ...
    def suma_totales(self, cuentas, currency, tasa):
        ...
        if currency.id == 2 or currency.id == 1:
            res_totales['credit_total'] = total_credit
            res_totales['debit_total'] = total_debit
            res_totales['balance_total'] = total_balance
    ...
    def generate_checking_balance(self):
        if self.report_format == '0':
            data = {
                'ids': self.ids,
                'model': 'report.locv_checking_balance.report_checking_balance',
                'form': {
            ...
            return self.env.ref('locv_checking_balance.action_report_checking_balance').report_action(self, data=data)
        else:
            unico = []
            today = datetime.now()
            hoy = date.today()
            ...
            start_date1 = datetime.strftime(datetime.strptime(str(self.start_date),DEFAULT_SERVER_DATE_FORMAT),format_new)
            end_date1 = datetime.strftime(datetime.strptime(str(self.end_date),DEFAULT_SERVER_DATE_FORMAT),format_new)
            hora = today.hour
            minute = today.minute
            if minute < 10:
            ...
            writer.write_merge(row, row, 2, 3, "Número de identificación Fiscal:", sub_header_style_bold)
            ...
            if self.target_movement == '0' and self.show_accounts == False:
                state = 'posted'

            # TODOS LOS ASIENTOS VALIDADOS CON SALDO DISTINTO A 0
            if self.target_movement == '0' and self.show_accounts == '1':
                state = 'posted'
                ...
                if self.balance == True:
                    ...
                    for line in a['move_lines']:
                        if line['lname'] == 'Initial Balance' or line['lname'] == 'Balance Inicial':
                            ...
                            writer.write_merge(row, row, 5, 6, line['balance'],line_content_style)
...
class ReportRetentionISLR(models.AbstractModel):
    _name = 'report.locv_checking_balance.report_checking_balance'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # TODOS LOS ASIENTOS VALIDADOS CON MOVIMIENTOS
        if target_movement == '0' and show_accounts == '0':
            state = 'posted'

        # TODOS LOS ASIENTOS VALIDADOS CON SALDO DISTINTO A 0
        if target_movement == '0' and show_accounts == '1':
            state = 'posted'
        ...
        return {
            ...
            'cuentas': cuentas,
            'suma': suma,
            'currency_line': currency_line.rate,
        }
    ...
    def amount_initial(self, account_account,show_accounts, balance,date_start,end_date,company,currency, state, currency_id1, tasa):
        ...
        for row in cr.dictfetchall():
            ...
            for line in move_lines.get(row['account_id']):
                #if line['debit'] != 0:
                if currency_id1 == 2 or currency_id1 == 1:
                    balance += (line['debit'] - line['credit'])
                else:
                    balance += (line['debit'] - line['credit'])
            row['balance'] += balance
        ...
        for account in account_account:
            ...
            for line in res.get('move_lines'):
                if currency_id1 == 2 or currency_id1 == 1:
                    res['debit'] += line['debit']/currency_line.rate if line['lname'] != 'Initial Balance' else False
                    res['credit'] += line['credit']/currency_line.rate if line['lname'] != 'Initial Balance' else False
                    res['balance'] = line['balance']/currency_line.rate
                else:
                    res['debit'] += line['debit'] if line['lname'] != 'Initial Balance' else False
                    res['credit'] += line['credit']  if line['lname'] != 'Initial Balance' else False
                    res['balance'] = line['balance']
    ...
    def suma_totales(self, cuentas, currency_id1,tasa):
        ...
        for account in cuentas:
            ...
            total_balance += account['balance']
        if  currency_id1 == 2 or currency_id1 == 1:
            res_totales['credit_total'] = total_credit/currency_line.rate
            res_totales['debit_total'] = total_debit/currency_line.rate
            res_totales['balance_total'] = total_balance/currency_line.rate
        else:
            res_totales['credit_total'] = total_credit
            res_totales['debit_total'] = total_debit
            res_totales['balance_total'] = total_balance
